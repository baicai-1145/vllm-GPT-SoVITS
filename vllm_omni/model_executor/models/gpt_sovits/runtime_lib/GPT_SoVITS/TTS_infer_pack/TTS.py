...
from runtime_preload import configure_model_runtime_environment, preload_text_runtime_deps

configure_model_runtime_environment()
...
from TTS_infer_pack.TextPreprocessor import StageLimiter, TextPreprocessor

resample_transform_dict = {}


def _verbose_runtime_text() -> bool:
...
def _compile_vits_dec_enabled() -> bool:
...
class TTS_Config:
    default_configs = {
        "v1": {
            ...
            "cnhuhbert_base_path": "GPT_SoVITS/pretrained_models/chinese-hubert-base",
            "bert_base_path": "GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large",
            "cnhuhbert_disable_cudnn_layer_indices": "",
    ...
    def __init__(self, configs: dict | str = None):
        ...
        if str(self.device) == "cpu" and self.precision_name != "fp32":
            print("Warning: Reduced precision is not supported on CPU, set precision to fp32.")
        ...
        self.cnhuhbert_disable_cudnn_layer_indices = self.configs.get(
            "cnhuhbert_disable_cudnn_layer_indices",
            self.default_configs[version].get("cnhuhbert_disable_cudnn_layer_indices", ""),
        )
        self.cnhuhbert_backend_preset = self.configs.get(
            "cnhuhbert_backend_preset",
            self.default_configs[version].get("cnhuhbert_backend_preset", ""),
        )
    ...
    def update_configs(self):
        self.config = {
            ...
            "cnhuhbert_disable_cudnn_layer_indices": self.cnhuhbert_disable_cudnn_layer_indices,
            "cnhuhbert_backend_preset": self.cnhuhbert_backend_preset,
        }
        return self.config
...
class TTS:
    @staticmethod
    def _resolve_ref_stage_slots(env_name: str, default: int) -> int:
        raw = os.environ.get(env_name)
        if raw is None:
            raw = os.environ.get("GPTSOVITS_PREPARE_REF_SLOTS")
        if raw is None:
            raw = str(default)
        return max(1, int(raw))

    @classmethod
    def _build_ref_stage_limiters(cls) -> tuple[StageLimiter, StageLimiter, bool]:
        legacy_raw = os.environ.get("GPTSOVITS_PREPARE_REF_SLOTS")
        semantic_raw = os.environ.get("GPTSOVITS_PREPARE_REF_SEMANTIC_SLOTS")
        spec_raw = os.environ.get("GPTSOVITS_PREPARE_REF_SPEC_SLOTS")

        # Preserve legacy behavior by default. The new split limiters should only
        # change runtime behavior when the dedicated env vars are explicitly set.
        if semantic_raw is None and spec_raw is None:
            shared_slots = max(1, int(legacy_raw or "4"))
            shared_limiter = StageLimiter(shared_slots)
            return shared_limiter, shared_limiter, False

        semantic_slots = cls._resolve_ref_stage_slots("GPTSOVITS_PREPARE_REF_SEMANTIC_SLOTS", 4)
        spec_slots = cls._resolve_ref_stage_slots("GPTSOVITS_PREPARE_REF_SPEC_SLOTS", 4)
        return StageLimiter(semantic_slots), StageLimiter(spec_slots), True
    ...
    def __init__(self, configs: dict | str | TTS_Config):
        ...
        (
            self.prepare_ref_semantic_stage_limiter,
            self.prepare_ref_spec_stage_limiter,
            self.prepare_ref_stage_limiters_split,
        ) = self._build_ref_stage_limiters()
        self.prepare_ref_audio_stage_limiter = self.prepare_ref_semantic_stage_limiter
        ...
        self.t2s_kv_cache_pool_state = None
        ...
        self._init_models()
        self.runtime_prepare_components_deferred = str(
            os.environ.get("GPTSOVITS_RUNTIME_SKIP_PREPARE_COMPONENTS", "0")
    ...
    def refresh_runtime_components(self):
        ...
        self.prepare_g2pw_batch_worker = None
        self.prepare_text_cpu_worker = None
        if getattr(self.t2s_model, "model", None) is not None:
            pool_state = attach_t2s_kv_cache_pool(self.t2s_model.model, self.configs.device)
            pool = getattr(self.t2s_model.model, "kv_cache_pool", None)
            self.t2s_kv_cache_pool_state = pool.snapshot() if pool is not None else dict(vars(pool_state))
        ...
        self._prewarm_prepare_ref_runtime()
    ...
    def _prewarm_g2pw_runtime(self) -> None:
        ...
        try:
            ...
            if hasattr(chinese2, "prewarm_g2pw_pipeline"):
                pipeline_prewarmed = bool(chinese2.prewarm_g2pw_pipeline(sentences=warm_texts, rounds=warm_rounds))
    ...
    def _prewarm_prepare_ref_runtime(self) -> None:
        worker = getattr(self, "prepare_ref_semantic_batch_worker", None)
        if worker is None:
            return
        if os.environ.get("GPTSOVITS_PREPARE_REF_PREWARM", "1") == "0":
            return
        warm_rounds = max(1, int(os.environ.get("GPTSOVITS_PREPARE_REF_PREWARM_ROUNDS", "1")))
        try:
            plans = self._build_prepare_ref_prewarm_plans(worker)
            if not plans:
                return
            warm_start = time.perf_counter()
            profiles = worker.prewarm(plans=plans, rounds=warm_rounds)
            warm_ms = (time.perf_counter() - warm_start) * 1000.0
            print(
                "Prewarmed ref semantic runtime: "
                f"workers={getattr(worker, 'worker_count', 1)} rounds={warm_rounds} "
                f"plans={len(plans)} total_ms={warm_ms:.2f}"
            )
            if os.environ.get("GPTSOVITS_PREPARE_REF_PREWARM_VERBOSE", "0") not in {"0", "false", "no", "off"}:
                for profile in profiles:
                    print(
                        "ref prewarm profile:",
                        {
                            "shard": int(profile.get("prompt_semantic_shard_index", -1)),
                            "target_samples": int(profile.get("prompt_semantic_prewarm_target_samples", 0)),
                            "target_batch_size": int(profile.get("prompt_semantic_prewarm_target_batch_size", 0)),
                            "forward_ms": float(profile.get("prompt_semantic_forward_ms", 0.0)),
                            "ssl_forward_ms": float(profile.get("prompt_semantic_ssl_forward_ms", 0.0)),
                            "extract_latent_ms": float(profile.get("prompt_semantic_extract_latent_ms", 0.0)),
                        },
                    )
        except Exception:
            traceback.print_exc()
    ...
    @staticmethod
    def _parse_ref_prewarm_csv_ints(raw: str | None) -> list[int]:
        values: list[int] = []
    ...
    def _select_evenly_spaced_indices(total: int, count: int) -> list[int]:
        ...
        if count == 1:
            return [total - 1]
        indices = {
            min(total - 1, max(0, int(round(position * (total - 1) / float(count - 1))))) for position in range(count)
        }
        return sorted(indices)

    def _build_prepare_ref_prewarm_plans(self, worker) -> list[dict]:
        shards = list(getattr(worker, "shards", []) or [])
        if not shards:
            return []
        shard0 = shards[0]
        zero_wav_samples = int(getattr(shard0, "zero_wav_samples", int(self.configs.sampling_rate * 0.3)))
        min_valid_samples = int(max(48000, 48000 + zero_wav_samples))
        prewarm_mode = str(os.environ.get("GPTSOVITS_PREPARE_REF_PREWARM_MODE", "runtime")).strip().lower() or "runtime"
        bucket_upper_bounds = [int(bound) for bound in getattr(shard0, "bucket_upper_bounds", []) or []]
        if not bucket_upper_bounds:
            bucket_upper_bounds = [min_valid_samples]
        explicit_lengths = self._parse_ref_prewarm_csv_ints(
            os.environ.get("GPTSOVITS_PREPARE_REF_PREWARM_TARGET_SAMPLES")
        )
        if prewarm_mode in {"single", "basic"} and not explicit_lengths:
            target_lengths = [min_valid_samples]
        elif explicit_lengths:
            target_lengths = sorted({max(min_valid_samples, value) for value in explicit_lengths})
        else:
            per_item_hot_ceiling = max(
                min_valid_samples,
                int(
                    math.ceil(
                        float(getattr(shard0, "max_batch_samples", min_valid_samples))
                        / float(max(1, getattr(shard0, "max_batch_items", 1)))
                    )
                ),
            )
            ratio_values = os.environ.get("GPTSOVITS_PREPARE_REF_PREWARM_TARGET_RATIOS", "0.0,0.625,1.01")
            snap_to_bucket = str(
                os.environ.get("GPTSOVITS_PREPARE_REF_PREWARM_SNAP_TO_BUCKETS", "1")
            ).strip().lower() not in {"0", "false", "no", "off"}
            desired_targets: list[int] = []
            for ratio_raw in str(ratio_values).split(","):
                ratio_raw = ratio_raw.strip()
                if not ratio_raw:
                    continue
                ratio = float(ratio_raw)
                if ratio <= 0.0:
                    desired_targets.append(min_valid_samples)
                else:
                    desired_targets.append(max(min_valid_samples, int(math.ceil(per_item_hot_ceiling * ratio))))
            if not desired_targets:
                desired_targets = [min_valid_samples, per_item_hot_ceiling]
            if snap_to_bucket:
                candidate_lengths = sorted({max(min_valid_samples, bound) for bound in bucket_upper_bounds})
                target_lengths = []
                for desired_target in desired_targets:
                    snapped = next(
                        (value for value in candidate_lengths if value >= desired_target), candidate_lengths[-1]
                    )
                    target_lengths.append(int(snapped))
                target_lengths = sorted(set(target_lengths))
            else:
                max_valid_samples = int(getattr(shard0, "max_batch_samples", 160000))
                target_lengths = sorted(
                    {max(min_valid_samples, min(max_valid_samples, int(value))) for value in desired_targets}
                )
        default_batch_sizes = [1]
        max_batch_items = max(1, int(getattr(shard0, "max_batch_items", 1)))
        if prewarm_mode not in {"single", "basic"} and max_batch_items > 2:
            default_batch_sizes.append(max(2, min(max_batch_items, int(math.ceil(float(max_batch_items) * 0.625)))))
        if prewarm_mode not in {"single", "basic"} and max_batch_items > 1:
            default_batch_sizes.append(max_batch_items)
        explicit_batch_sizes = self._parse_ref_prewarm_csv_ints(
            os.environ.get("GPTSOVITS_PREPARE_REF_PREWARM_BATCH_SIZES")
        )
        target_batch_sizes = sorted(
            {
                max(1, min(max_batch_items, value))
                for value in (explicit_batch_sizes if explicit_batch_sizes else default_batch_sizes)
            }
        )
        raw_sr = 16000
        raw_audio = torch.zeros((1, 48000), dtype=torch.float32)
        plans: list[dict] = []
        for target_samples in target_lengths:
            wav16k = torch.zeros(int(target_samples), dtype=torch.float32).contiguous()
    ...
    def snapshot_prepare_runtime_components(self) -> dict:
        ...
        return {
            ...
            "ref_semantic": {
                "stage_limiter": dict(self.prepare_ref_semantic_stage_limiter.snapshot()),
                "batch_worker": (
                    None
            ...
            "ref_spec": {
                "stage_limiter": dict(self.prepare_ref_spec_stage_limiter.snapshot()),
            },
            "ref_audio_limiters": {
                "split": bool(getattr(self, "prepare_ref_stage_limiters_split", False)),
                "shared_legacy_limiter": bool(
                    self.prepare_ref_semantic_stage_limiter is self.prepare_ref_spec_stage_limiter
                ),
            },
            "text_preprocessor": (
                None
                if self.text_preprocessor is None or not hasattr(self.text_preprocessor, "snapshot")
                else self.text_preprocessor.snapshot()
            ),
            "g2pw": g2pw_runtime,
            "g2pw_batch": (
    ...
    def init_cnhuhbert_weights(self, base_path: str):
        ...
        backend_preset = str(self.configs.cnhuhbert_backend_preset or "").strip()
        if backend_preset and "GPTSOVITS_CNHUBERT_BACKEND_PRESET" not in os.environ:
            os.environ["GPTSOVITS_CNHUBERT_BACKEND_PRESET"] = backend_preset
        disable_cudnn_layers = str(self.configs.cnhuhbert_disable_cudnn_layer_indices or "").strip()
        if (
            disable_cudnn_layers
            and str(self.configs.device) != "cpu"
            and "GPTSOVITS_CNHUBERT_DISABLE_CUDNN_LAYER_INDICES" not in os.environ
        ):
    ...
    def _maybe_compile_vits_decoder(self) -> None:
        if self.configs.use_vocoder or not _compile_vits_dec_enabled():
            return
        if not hasattr(torch, "compile") or getattr(self.vits_model, "_compiled_dec", None) is not None:
            return
        compile_mode = (
            os.environ.get("GPTSOVITS_COMPILE_VITS_DEC_MODE", "max-autotune-no-cudagraphs").strip()
            or "max-autotune-no-cudagraphs"
        )
        dynamic = os.environ.get("GPTSOVITS_COMPILE_VITS_DEC_DYNAMIC", "1").strip().lower() not in {
            "0",
            "false",
            "no",
            "off",
        ...
        skip_dynamic_graphs = os.environ.get(
            "GPTSOVITS_COMPILE_VITS_DEC_SKIP_DYNAMIC_CUDAGRAPH", "0"
        ).strip().lower() not in {
            "0",
            "false",
            "no",
        ...
        try:
            ...
            self.vits_model._compiled_dec = torch.compile(
                self.vits_model.dec,
                mode=compile_mode,
                dynamic=dynamic,
            )
            ...
            print(
                f"Compiled VITS decoder: mode={compile_mode} dynamic={dynamic} "
    ...
    def _maybe_compile_t2s_prealloc_decoder(self) -> None:
        ...
        dynamic = os.environ.get("GPTSOVITS_COMPILE_T2S_PREALLOC_DYNAMIC", "1").strip().lower() not in {
            "0",
            "false",
            "no",
            "off",
        ...
        capture_scalar_outputs = os.environ.get(
            "GPTSOVITS_COMPILE_T2S_PREALLOC_CAPTURE_SCALAR_OUTPUTS", "1"
        ).strip().lower() not in {
            "0",
            "false",
            "no",
        ...
        skip_dynamic_graphs = os.environ.get(
            "GPTSOVITS_COMPILE_T2S_PREALLOC_SKIP_DYNAMIC_CUDAGRAPH", "0"
        ).strip().lower() not in {
            "0",
            "false",
            "no",
        ...
        try:
            ...
            elapsed_ms = (time.perf_counter() - started_at) * 1000.0
            print(
                f"Compiled T2S prealloc decode: mode={compile_mode} dynamic={dynamic} "
                f"capture_scalar_outputs={capture_scalar_outputs} "
                f"skip_dynamic_cudagraph={skip_dynamic_graphs} wrap_ms={elapsed_ms:.2f}"
            )
        except Exception as exc:
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        # In inference, decoder weight_norm adds per-forward reparameterization overhead
        # on the hottest synth path and can be removed without changing outputs.
        if (
            not self.configs.use_vocoder
            and hasattr(vits_model, "dec")
            and hasattr(vits_model.dec, "remove_weight_norm")
        ):
    ...
    def _prepare_ref_prompt_wav16k_for_worker(self, raw_audio: torch.Tensor, raw_sr: int):
        if self.prepare_ref_semantic_batch_worker is None or str(
        ...
        wav16k, cpu_prepare_ms, cpu_prepare_limiter_stats = self._prepare_prompt_semantic_wav16k_profile(
            raw_audio, raw_sr
        )
        return wav16k, {
            "prompt_semantic_cpu_prepare_wait_ms": float(cpu_prepare_limiter_stats.get("wait_ms", 0.0)),
            "prompt_semantic_cpu_prepare_slots": float(cpu_prepare_limiter_stats.get("slots", 0.0)),
            "prompt_semantic_cpu_prepare_inflight_peak": float(cpu_prepare_limiter_stats.get("peak_inflight", 0.0)),
    ...
    def extract_ref_audio_bundle(self, ref_audio_path: str):
        ...
        if self.prepare_ref_semantic_batch_worker is None:
            ...
            with self.prepare_ref_semantic_stage_limiter.enter() as prompt_semantic_limiter_stats:
                prompt_semantic_start = time.perf_counter()
                prompt_semantic, prompt_semantic_runtime_profile = (
                    self._extract_prompt_semantic_profile_from_prepared_wav16k(wav16k)
                )
                prompt_semantic_ms = (time.perf_counter() - prompt_semantic_start) * 1000.0
            with self.prepare_ref_spec_stage_limiter.enter() as ref_spec_limiter_stats:
            ...
            audio_stage_wait_ms = float(prompt_semantic_limiter_stats["wait_ms"]) + float(
                ref_spec_limiter_stats["wait_ms"]
            )
            audio_stage_slots = max(
                float(prompt_semantic_limiter_stats["slots"]),
                float(ref_spec_limiter_stats["slots"]),
            )
            audio_stage_inflight_peak = max(
                float(prompt_semantic_limiter_stats["peak_inflight"]),
                float(ref_spec_limiter_stats["peak_inflight"]),
            )
            prompt_semantic_profile = {
                "prompt_semantic_wait_ms": float(prompt_semantic_limiter_stats["wait_ms"]),
                ...
                "prompt_semantic_stage_limiter_wait_ms": float(prompt_semantic_limiter_stats["wait_ms"]),
                ...
                "prompt_semantic_stage_slots": float(prompt_semantic_limiter_stats["slots"]),
                "prompt_semantic_stage_inflight_peak": float(prompt_semantic_limiter_stats["peak_inflight"]),
        ...
        if self.prepare_ref_semantic_batch_worker is not None:
            ...
            prompt_semantic, worker_profile = self.prepare_ref_semantic_batch_worker.submit(
                raw_audio, raw_sr, wav16k=wav16k
            )
            prompt_semantic_profile.update(worker_profile)
            prompt_semantic_profile["prompt_semantic_cpu_prepare_wait_ms"] = float(
                prompt_semantic_profile.get("prompt_semantic_cpu_prepare_wait_ms", 0.0)
                + local_cpu_prepare_profile.get("prompt_semantic_cpu_prepare_wait_ms", 0.0)
            )
            prompt_semantic_profile["prompt_semantic_cpu_prepare_slots"] = float(
                max(
                    prompt_semantic_profile.get("prompt_semantic_cpu_prepare_slots", 0.0),
                    local_cpu_prepare_profile.get("prompt_semantic_cpu_prepare_slots", 0.0),
                )
            )
            prompt_semantic_profile["prompt_semantic_cpu_prepare_inflight_peak"] = float(
                max(
                    prompt_semantic_profile.get("prompt_semantic_cpu_prepare_inflight_peak", 0.0),
                    local_cpu_prepare_profile.get("prompt_semantic_cpu_prepare_inflight_peak", 0.0),
                )
            )
            prompt_semantic_profile["prompt_semantic_cpu_prepare_ms"] = float(
                prompt_semantic_profile.get("prompt_semantic_cpu_prepare_ms", 0.0)
    ...
    async def extract_ref_audio_bundle_async(self, ref_audio_path: str):
        ...
        load_ms = (time.perf_counter() - load_start) * 1000.0
        load_done_at = time.perf_counter()
        wav16k_prepare_task = asyncio.create_task(
            asyncio.to_thread(self._prepare_ref_prompt_wav16k_for_worker, raw_audio, raw_sr)
        )

        async def _submit_prompt_semantic():
            wav16k, local_cpu_prepare_profile = await wav16k_prepare_task
            prompt_submit_at = time.perf_counter()
            prompt_semantic, prompt_semantic_profile = await self.prepare_ref_semantic_batch_worker.submit_async(
                raw_audio,
                raw_sr,
                wav16k=wav16k,
            )
            ...
            return prompt_semantic, prompt_semantic_profile, prompt_submit_at

        prompt_semantic_task = asyncio.create_task(_submit_prompt_semantic())
        ...
        (
            (prompt_semantic, prompt_semantic_profile, prompt_submit_at),
            (refer_spec, ref_spec_profile),
        ...
        return {
            ...
            "profile": {
                ...
                "prompt_semantic_submit_after_load_ms": float(max(0.0, (prompt_submit_at - load_done_at) * 1000.0)),
                "prompt_semantic_worker_queue_wait_ms": float(
                    prompt_semantic_profile.get("prompt_semantic_worker_queue_wait_ms", 0.0)
                ),
                "prompt_semantic_batch_collect_wait_ms": float(
                ...
                "bundle_total_ms": float(
                    load_ms + audio_stage_wait_ms + prompt_semantic_ms + ref_spec_profile.get("ref_spec_ms", 0.0)
                ),
            },
        }
    ...
    def run(self, inputs: dict):
        ...
        chunk_split_threshold = 0.0  # 该值代表语义token与mute token的余弦相似度阈值，若大于该阈值，则视为可切分点。
            ...
            for item in data:
                ...
                if not streaming_mode:
                    ...
                    semantic_token_generator = self.t2s_model.model.infer_panel(
                        ...
                        chunk_split_threshold=chunk_split_threshold,
                    )
                    t4 = time.perf_counter()
                    t_34 += t4 - t3
    ...
    def synthesize_audio_request_local(
        ...
        if not self.configs.use_vocoder:
            refer_audio_spec_list = [
                item[0].to(dtype=self.precision, device=self.configs.device) for item in refer_specs
            ]
            sv_emb = None
            if self.is_v2pro:
                sv_emb = []
    ...
    def synthesize_audio_requests_local_batched(
        ...
        shared_runtime_cache: dict | None = None,
    ) -> list[torch.Tensor]:
        ...
        shared_audio_tensor = None
        shared_cache_key = None
        if refer_specs:
            first_group = list(refer_specs[0]) if isinstance(refer_specs[0], list) else [refer_specs[0]]
            if len(first_group) == 1:
                first_spec, first_audio_tensor = first_group[0]
                first_spec_key = (
                ...
                if first_audio_tensor is not None:
                    first_audio_key = (
                        str(first_audio_tensor.device),
                        str(first_audio_tensor.dtype),
                        tuple(int(item) for item in first_audio_tensor.shape),
                        int(first_audio_tensor.data_ptr()),
                    )
                shared_single_refer = True
                for refer_spec_item in refer_specs[1:]:
                ...
                if shared_single_refer:
                    shared_refer_audio_spec = first_spec.to(dtype=self.precision, device=device)
                    shared_audio_tensor = first_audio_tensor
                    shared_cache_key = (
                        "shared_single_refer",
                        int(first_spec.data_ptr()),
                        None if first_audio_tensor is None else int(first_audio_tensor.data_ptr()),
                        str(device),
                        str(self.precision),
                    )
        ...
        cached_entry = None
        if shared_single_refer and shared_runtime_cache is not None and shared_cache_key is not None:
            cached_entry = shared_runtime_cache.get(shared_cache_key)

        if self.is_v2pro:
            if shared_single_refer:
                if shared_audio_tensor is None:
                    raise ValueError(i18n("v2Pro request-local batched synthesis 缺少 16k 参考音频"))
                if cached_entry is not None and cached_entry.get("sv_emb") is not None:
                    sv_emb_batch = cached_entry["sv_emb"]
                else:
                    sv_emb_batch = self.sv_model.compute_embedding3(shared_audio_tensor).to(device)
            else:
                sv_emb_batch = torch.cat(sv_emb_list, dim=0)
        if shared_single_refer:
            if shared_refer_audio_spec is None:
                raise ValueError("shared_single_refer 检测成功但 refer_audio_spec 缺失")
            refer_audio_specs = [shared_refer_audio_spec]

        precomputed_ge = None
        if shared_single_refer:
            if cached_entry is not None and cached_entry.get("ge") is not None:
                precomputed_ge = cached_entry["ge"]
            else:
                ...
                if self.vits_model.version == "v1":
                    precomputed_ge = self.vits_model.ref_enc(shared_refer_audio_spec * refer_mask, refer_mask)
                else:
                    precomputed_ge = self.vits_model.ref_enc(shared_refer_audio_spec[:, :704] * refer_mask, refer_mask)
                if self.is_v2pro:
                    precomputed_ge = precomputed_ge + self.vits_model.sv_emb(sv_emb_batch[:1]).unsqueeze(-1)
                    precomputed_ge = self.vits_model.prelu(precomputed_ge)
                if shared_runtime_cache is not None and shared_cache_key is not None:
                    shared_runtime_cache[shared_cache_key] = {
                        "sv_emb": sv_emb_batch,
                        "ge": precomputed_ge,
                    }
