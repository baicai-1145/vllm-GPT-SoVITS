...
class WorkerSubmitLifecycleMixin:
    ...
    def snapshot(self) -> dict:
        with self.condition:
            ...
            kv_pool = getattr(model, "kv_cache_pool", None)
            kv_pool_state = (
                dict(kv_pool.snapshot())
                if kv_pool is not None and hasattr(kv_pool, "snapshot")
                else dict(getattr(self.tts, "t2s_kv_cache_pool_state", {}) or {})
            )
            ...
            return {
                ...
                **finalize_state,
                ...
                "t2s_kv_cache_pool": kv_pool_state,
    ...
    async def prepare_direct_shared_segments_profiled_async(
        ...
        return await self.prepare_executor.prepare_direct_shared_segments_profiled_async(specs)
