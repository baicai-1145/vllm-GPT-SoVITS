...
class EngineDispatchStageMixin:
    def _kv_pool_blocks_dispatch(self, worker_state: Dict[str, object]) -> bool:
        kv_pool_state = dict(worker_state.get("t2s_kv_cache_pool") or {})
        if not bool(kv_pool_state.get("enabled", False)):
            return False
        if not self.scheduler_worker.is_engine_decode_control_enabled():
            return False
        capacity = int(kv_pool_state.get("max_batch_size", 0))
        if capacity <= 0:
            return False
        decode_runtime_state = self.snapshot_engine_decode_runtime_state()
        inflight_decode_rows = int(decode_runtime_state.get("active_request_count", 0)) + int(
            decode_runtime_state.get("pending_jobs", 0)
        )
        return inflight_decode_rows >= capacity
    ...
    def run_engine_dispatch_once(self, policy_snapshot: Dict[str, object], worker_state: Dict[str, object]) -> bool:
        if not bool(policy_snapshot.get("allowed", True)):
            return False
        if self._kv_pool_blocks_dispatch(worker_state):
            return False
        dispatch_task = self.dispatch_queue_owner.pop_left()
        if dispatch_task is None:
            return False
        if self._kv_pool_blocks_dispatch(worker_state):
            self.dispatch_queue_owner.enqueue_left(dispatch_task)
            return False
