...
class EngineRuntimeFacade:
    ...
    def _t2s_kv_pool_snapshot(self) -> Dict[str, Any]:
        model = getattr(getattr(self.tts, "t2s_model", None), "model", None)
        pool = getattr(model, "kv_cache_pool", None)
        if pool is not None and hasattr(pool, "snapshot"):
            try:
                return dict(pool.snapshot())
            except Exception:
                pass
        return dict(getattr(self.tts, "t2s_kv_cache_pool_state", {}) or {})
    ...
    def _build_stage_summary(
        ...
        return {
            **counters,
            ...
            "t2s_kv_cache_pool": self._t2s_kv_pool_snapshot(),
        }
    ...
    def get_runtime_state(self) -> dict:
        ...
        return {
            "message": "success",
            "runtime_cache_env": dict(getattr(self.owner, "runtime_cache_env", {}) or {}),
            "startup_prewarm": dict(getattr(self.owner, "startup_prewarm_summary", {}) or {}),
            "default_reference": {
