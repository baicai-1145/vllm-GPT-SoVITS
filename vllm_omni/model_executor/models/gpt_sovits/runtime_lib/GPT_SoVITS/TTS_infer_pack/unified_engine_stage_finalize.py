...
class EngineFinalizeStageMixin:
    @staticmethod
    def _engine_finalize_effective_batch_max_items(batch_max_items: int) -> int:
        batch_max_items = max(1, int(batch_max_items))
        raw = os.environ.get("GPTSOVITS_ENGINE_FINALIZE_QUOTA_ITEMS")
        if raw in [None, ""]:
            return batch_max_items
        return max(1, min(batch_max_items, int(raw)))

    def _build_finalize_task_group_key(self, task: SchedulerFinalizeTask):
        job = self.get_engine_job(task.request_id)
        if job is None:
            return None
        direct_parent_request_id = None
        try:
            direct_parent_request_id = self.scheduler_worker.api.direct_flow._extract_direct_parent_request_id(task.request_id)
        except Exception:
            direct_parent_request_id = None
        refer_spec = getattr(job.state, "refer_spec", None)
        refer_shapes = None
        if refer_spec is not None:
            refer_shapes = (
                tuple(refer_spec[0].shape),
                None if refer_spec[1] is None else tuple(refer_spec[1].shape),
            )
        return (
            str(direct_parent_request_id or job.engine_request_id or ""),
            int(len(list(getattr(job.state, "aux_refer_specs", []) or []))),
            refer_shapes,
            float(job.speed_factor),
            int(job.sample_steps),
        )

    def _get_finalize_task_wait_override_s(self, task: SchedulerFinalizeTask) -> float:
        direct_parent_request_id = None
        try:
            direct_parent_request_id = self.scheduler_worker.api.direct_flow._extract_direct_parent_request_id(task.request_id)
        except Exception:
            direct_parent_request_id = None
        if direct_parent_request_id in [None, ""]:
            return 0.0
        wait_ms = float(os.environ.get("GPTSOVITS_DIRECT_FINALIZE_BATCH_WAIT_MS", "8"))
    ...
    def take_engine_finalize_batch_nonblocking(self) -> List[SchedulerFinalizeTask]:
        ...
        batch_max_items = self._engine_finalize_effective_batch_max_items(
            int(finalize_policy.get("finalize_batch_max_items", 1))
        )
        return self.finalize_queue_owner.take_finalize_batch(
            ...
            task_group_key=self._build_finalize_task_group_key,
            task_wait_override_s=self._get_finalize_task_wait_override_s,
        )
