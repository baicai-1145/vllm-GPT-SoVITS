...
class EngineTaskQueueOwner:
    ...
    def enqueue_left(self, item: Any) -> None:
        with self.condition:
    ...
    def take_finalize_batch(
        ...
        *,
        ...
        batch_wait_s: float,
        use_vocoder: bool,
        task_group_key: Callable[[SchedulerFinalizeTask], Any] | None = None,
        task_wait_override_s: Callable[[SchedulerFinalizeTask], float] | None = None,
        ...
        with self.condition:
            ...
            effective_batch_wait_s = float(batch_wait_s)
            if task_wait_override_s is not None:
                effective_batch_wait_s = max(effective_batch_wait_s, float(task_wait_override_s(first_task)))
            if len(self.queue) + 1 < batch_max_items and oldest_age_s < effective_batch_wait_s:
                self.queue.appendleft(first_task)
                return []
            first_group_key = None if task_group_key is None else task_group_key(first_task)
            batch_deadline = time.perf_counter() + effective_batch_wait_s
            while len(selected_tasks) < batch_max_items:
                if first_group_key is not None:
                    matched_index = None
                    for index, task in enumerate(self.queue):
                        if task_group_key(task) == first_group_key:
                            matched_index = index
                            break
...
class EngineDecodeRuntimeOwner:
    ...
    def summarize_active_batch(active_batch: T2SActiveBatch | None) -> Dict[str, Any]:
        ...
        return {
            ...
            "kv_cache_pooled": bool(getattr(active_batch, "kv_cache_pooled", False)),
            "kv_cache_capacity": int(getattr(active_batch, "kv_cache_capacity", 0)),
            "kv_cache_batch_capacity": int(getattr(active_batch, "kv_cache_batch_capacity", 0)),
        }
