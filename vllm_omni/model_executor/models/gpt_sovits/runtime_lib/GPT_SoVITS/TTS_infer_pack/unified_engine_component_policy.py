...
class EnginePolicyArbiterController:
    ...
    def build_stage_counters(
        ...
        kv_pool_state = dict(worker_state.get("t2s_kv_cache_pool") or {})
        kv_pool_enabled = bool(kv_pool_state.get("enabled", False))
        kv_pool_capacity = int(kv_pool_state.get("max_batch_size", 0))
        kv_pool_active_rows = int(kv_pool_state.get("active_rows", 0))
        kv_pool_available_rows = int(kv_pool_state.get("available_rows", max(0, kv_pool_capacity - kv_pool_active_rows)))
        return {
            ...
            "kv_pool_enabled": kv_pool_enabled,
            "kv_pool_capacity": kv_pool_capacity,
            "kv_pool_active_rows": kv_pool_active_rows,
            "kv_pool_available_rows": kv_pool_available_rows,
            "decode_backlog": int(
    ...
    def build_policy_snapshot(
        ...
        return {
            ...
            "metrics": {
                ...
                "kv_pool_enabled": bool(counters.get("kv_pool_enabled", False)),
                "kv_pool_capacity": int(counters.get("kv_pool_capacity", 0)),
                "kv_pool_active_rows": int(counters.get("kv_pool_active_rows", 0)),
                "kv_pool_available_rows": int(counters.get("kv_pool_available_rows", 0)),
            },
    ...
    def select_stage(self) -> tuple[str, str, Dict[str, Any], Dict[str, Any]]:
        ...
        kv_pool_enabled = bool(kv_pool_state.get("enabled", False))
        kv_pool_capacity = int(kv_pool_state.get("max_batch_size", 0))
        kv_pool_dispatch_allowed = (
            (not kv_pool_enabled)
            or kv_pool_capacity <= 0
            or (worker_running_requests + worker_pending_jobs) < kv_pool_capacity
        )
        if (
        ...
        if finalize_waiting > 0 and (decode_waiting <= 0 or not policy_allowed or decode_budget_remaining <= 0 or not kv_pool_dispatch_allowed):
        ...
        if prepare_waiting > 0 and (decode_waiting <= 0 or not policy_allowed or decode_budget_remaining <= 0 or not kv_pool_dispatch_allowed):
        ...
        if decode_waiting > 0 and policy_allowed and kv_pool_dispatch_allowed:
